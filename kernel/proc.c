...
int maximum_ticks[5] = {1, 2, 4, 8, 16};
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE && (begin_time == -1 || begin_time < p->ctime ) )
      {
...
void procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    printf("%d %d %s %d %d %d", p->pid, p->dynamic_priority, state, p->rtime, wtime, p->scheduled);
    printf("\n");
#else
#ifdef MLFQ
    // acquire(&p->lock);
